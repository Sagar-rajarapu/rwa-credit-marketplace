...
impl LoanPoolContract {
    ...
    pub fn fund(env: Env, loan_id: u32, investor: Address, amount: i128) {
        ...
        let mut loan: Loan = env.storage().persistent().get(&(LOAN, loan_id)).unwrap();
    ...
    pub fn repay(env: Env, loan_id: u32, borrower: Address, amount: i128) {
        borrower.require_auth();
        let mut loan: Loan = env.storage().persistent().get(&(LOAN, loan_id)).unwrap();
        assert!(loan.status == LoanStatus::Active || loan.status == LoanStatus::Repaying, "not active");
        assert!(loan.borrower == borrower, "not borrower");

        loan.repaid_amount += amount;
        loan.status = LoanStatus::Repaying;
        let total_due = loan.funded_amount + (loan.funded_amount * loan.interest_bps as i128 / 10_000);
    ...
    /// Returns investor's claimable yield share (read-only).
    pub fn yield_share(env: Env, loan_id: u32, investor: Address) -> i128 {
        let loan: Loan = env.storage().persistent().get(&(LOAN, loan_id)).unwrap();
        ...
        let total_interest = loan.funded_amount * loan.interest_bps as i128 / 10_000;
    ...
    pub fn get_loan(env: Env, loan_id: u32) -> Loan {
        env.storage().persistent().get(&(LOAN, loan_id)).unwrap()
    }
}
