...
router.get('/', async (req, res) => {
  ...
  const { rows } = await db.query(
    'SELECT i.*, l.title, l.interest_bps, l.duration_days FROM investments i JOIN listings l ON i.listing_id = l.listing_id WHERE i.investor_address = $1 ORDER BY i.created_at DESC',
    [investor]
  );
  res.json(rows);
});
